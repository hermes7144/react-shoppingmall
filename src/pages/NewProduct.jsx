  ...
  const handleSubmit = (e) => {
    ...
    uploadImage(file)
      .then((url) => {
        ...
        // Fierbases에 새로운 제품 추가함
      })
      .finally(() => setIsUploading(false));
    // 제품의 사진을 Coudinary에 업로드하고 URL을 획득
  };
  ...
  return (
    <section className="w-full text-center">
      ...
      <form className="flex flex-col px-12" onSubmit={handleSubmit}>
        ...
        <input type="text" name="description" value={product.description ?? ''} placeholder="제품 설명" onChange={handleChange} />
        <input type="text" name="options" value={product.options ?? ''} placeholder="옵션들(콤마(,)로 구분)" onChange={handleChange} />
